...
import com.example.susa.SearchResources;
import com.google.gson.JsonArray;
import com.google.gson.JsonObject;

public class SearchCatAdapter2 extends RecyclerView.Adapter<SearchCatAdapter2.ViewHolder> {
    ...
    public SearchCatAdapter2(JsonArray ja, Context context , SearchResources searchResources, String selected)  {
        this.ja = ja;
    ...
    @NonNull
    @Override
    public SearchCatAdapter2.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.cat_item, parent, false);
        return new SearchCatAdapter2.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull SearchCatAdapter2.ViewHolder holder, int position) {
...
//            holder.card_click.setCardBackgroundColor(Color.WHITE);
    ...
    private void onItemClick(int position) {
        // Handle the item click event here (e.g., open a new activity or perform an action)
        searchResources.filter_cat(ja.get(position).getAsJsonObject().get("_id").getAsString());
